...
export default class QuotationsController {
    ...
    async store({ request, response }) {
        ...
        const customerProductIds = customer.products.map((p) => p.id);
    ...
    async approveQuotation({ params, response, auth }) {
        try {
            const quotation = await Quotation.find(params.id);
            if (!quotation) {
                return response.notFound({ message: 'Quotation tidak ditemukan' });
    ...
    async rejectQuotation({ params, response, auth }) {
        try {
            const quotation = await Quotation.find(params.id);
            if (!quotation) {
                return response.notFound({ message: 'Quotation tidak ditemukan' });
    ...
    async getQuotationDetails({ params, response }) {
        try {
            const quotation = await Quotation.query()
                .where('id', params.id)
                .preload('customer', (customerQuery) => {
...
//# sourceMappingURL=quotations_controller.js.map