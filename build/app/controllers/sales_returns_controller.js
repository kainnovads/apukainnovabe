...
export default class SalesReturnsController {
    ...
    async approveSalesReturn({ params, response, auth }) {
        const trx = await db.transaction();
        try {
    ...
    async rejectSalesReturn({ params, response, auth }) {
        try {
            const sr = await SalesReturn.find(params.id);
            if (!sr) {
                return response.notFound({ message: 'SalesReturn tidak ditemukan' });
    ...
    async getSalesReturnDetails({ params, response }) {
        try {
            const sr = await SalesReturn.query()
                .where('id', params.id)
                .preload('salesReturnItems', (query) => {
    ...
    async getSalesOrder({ params, response }) {
        try {
...
//# sourceMappingURL=sales_returns_controller.js.map