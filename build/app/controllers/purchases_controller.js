...
export default class PurchasesController {
    ...
    async store({ request, response }) {
        ...
        if (payload.attachment && payload.attachment instanceof MultipartFile) {
            try {
                ...
                const uploadResult = await this.storageService.uploadFile(payload.attachment, 'purchase_orders', true);
    ...
    async approvePurchaseOrder({ params, response, auth }) {
        try {
    ...
    async rejectPurchaseOrder({ params, response, auth }) {
        try {
            const po = await PurchaseOrder.find(params.id);
            if (!po) {
                return response.notFound({ message: 'PurchaseOrder tidak ditemukan' });
    ...
    async getPurchaseOrderDetails({ params, response }) {
        try {
            const po = await PurchaseOrder.query()
                .where('id', params.id)
                .preload('vendor', (vendorQuery) => {
