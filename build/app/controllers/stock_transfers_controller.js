...
export default class StockTransfersController {
    async index({ request, response }) {
        try {
            ...
            const includeDetails = request.input('includeDetails', false);
    ...
    async getStockTransferDetails({ params, response }) {
        try {
        ...
        try {
            ...
            for (const item of payload.stockTransferDetails) {
                ...
                if (!stock || stock.quantity < item.quantity) {
                    ...
                    await trx.rollback();
                    return response.badRequest({
                        message: `Stok untuk produk "${productName}" di gudang asal tidak mencukupi. Stok tersedia: ${stock?.quantity || 0}, kuantitas diminta: ${item.quantity}.`,
                    });
                }
            }
    ...
    async approveStockTransfer({ params, auth, response }) {
        const trx = await db.transaction();
        try {
    ...
    async rejectStockTransfer({ params, response, auth }) {
        try {
            const transfer = await StockTransfer.find(params.id);
            if (!transfer) {
                return response.notFound({ message: 'Transfer tidak ditemukan' });
    ...
    async getTotalStockTransfer({ response }) {
        try {
...
//# sourceMappingURL=stock_transfers_controller.js.map